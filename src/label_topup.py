...
CSV = ROOT / 'golden_set' / 'golden_set_topup_to_review.csv'
...
def header(idx, total, row):
    # Safe handling of NaN/float in hint columns
    hint_v2 = row.get('intent_llm_guess_v2', '')
    hint_v1 = row.get('intent_llm_guess', '')
    if not isinstance(hint_v2, str) or not hint_v2.strip():
        hint_v2 = ''
    if not isinstance(hint_v1, str) or not hint_v1.strip():
        hint_v1 = ''
    llm_hint = hint_v2 or hint_v1 or '?'
