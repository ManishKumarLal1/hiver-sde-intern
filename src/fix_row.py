...
NEW_INTENT = sys.argv[2] if len(sys.argv) > 2 else 'refund_return'
NEW_ESC    = sys.argv[3] if len(sys.argv) > 3 else 'escalate'
NEW_REASON = sys.argv[4] if len(sys.argv) > 4 else 'repeat_failure'
NEW_DIFF   = sys.argv[5] if len(sys.argv) > 5 else 'hard'
